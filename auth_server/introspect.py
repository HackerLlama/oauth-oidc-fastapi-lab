...
def introspect(
    ...
    Caller must provide client_id; must be a registered client (lab: no client_secret yet).
    """
    ...
    client = db.query(Client).filter(Client.client_id == client_id.strip()).first()
    if not client:
        raise HTTPException(
            status_code=401,
